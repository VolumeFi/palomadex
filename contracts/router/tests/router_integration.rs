...
use palomadex::router::{ExecuteMsg, InstantiateMsg, SwapOperation, SwapResponseData};
use palomadex_router::error::ContractError;
...
fn router_contract() -> Box<dyn Contract<Empty>> {
    Box::new(
        ContractWrapper::new_with_empty(
            palomadex_router::contract::execute,
            palomadex_router::contract::instantiate,
            palomadex_router::contract::query,
        )
        .with_reply_empty(palomadex_router::contract::reply),
    )
}
...
fn router_does_not_enforce_spread_assertion() {
    ...
    let router = app
        .instantiate_contract(
            ...
            owner.clone(),
            &InstantiateMsg {
                palomadex_factory: helper.factory.to_string(),
            },
            &[],
            "router",
            None,
    ...
    let err = app
        ...
        .unwrap_err();
    assert_eq!(
        palomadex_pair::error::ContractError::MaxSpreadAssertion {},
        err.downcast().unwrap()
    )
}
...
fn route_through_pairs_with_natives() {
    ...
    assert_eq!(resp_data.return_amount.u128(), 32_258_064517);
...
fn test_swap_route() {
    ...
    the same as the previous ask asset; contract2 --> contract3 --> contract4"
        )
    );
    ...
    assert_eq!(simulate_res.amount, Uint128::new(9996004));
    println!(
    ...
    let balance_res: BalanceResponse = app
        ...
        .unwrap();
    assert_eq!(balance_res.balance, Uint128::new(19992009));
